...
import { doc, getDoc, setDoc, updateDoc, query, collection, where, getDocs, writeBatch } from 'firebase/firestore';
...
export function AuthProvider({ children }: { children?: React.ReactNode }) {
    ...
    const updateUser = useCallback(async (updatedData: Partial<User>) => {
        ...
        // Construct payload with all potential fields to update, including avatarUrl
        const payload: any = {};
        ...
        // --- MANUAL PROPAGATION TO CLASSES ---
        // Since we denormalize student data in 'classes', we must update it everywhere
                ...
                if (!classesSnap.empty) {
                    ...
                    classesSnap.docs.forEach(classDoc => {
                        ...
                        if (classData.students && Array.isArray(classData.students)) {
                            // Find and update the specific student object within the array
                            const updatedStudentsList = classData.students.map((s: any) => {
                                if (s.id === user.id) {
                                    return {
                                        ...s,
                                        name: updatedData.name || s.name,
                                        avatarUrl: updatedData.avatarUrl || s.avatarUrl
                                    };
                                }
                                return s;
                            });
                            ...
                            batch.update(classDoc.ref, { students: updatedStudentsList });
                        }
                    });
                    
                    await batch.commit();
                    console.log("Profile changes propagated to classes.");
                }
            } catch (err) {
                console.error("Failed to propagate profile changes to classes:", err);
